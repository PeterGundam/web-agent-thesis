...
const MODEL = 'llm3';
const DELAY_BETWEEN_REQUESTS_MS = 4500; // Vorgabe: 10-15 Anfragen/Minute, keine parallelen Anfragen
...
// Durchsucht output_real_sites (und optional output_mind2web, falls vorhanden)
const dirs = [
  ...findSubsetDirs('output_real_sites'),
  ...findSubsetDirs('output_mind2web'),
];
...
// Toleranter Vergleich: Groß/Kleinschreibung und Sonderzeichen ignorieren
...
(async () => {
  if (dirs.length === 0) {
    console.error(`❌ Keine Testfälle mit ground_truth.json in output_real_sites/ oder output_mind2web/ gefunden.`);
    process.exit(1);
  }
  ...
  fs.writeFileSync('comparison_results_real.json', JSON.stringify(results, null, 2));
  console.log('\nDetails gespeichert in comparison_results_real.json');
})();
